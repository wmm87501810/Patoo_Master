package com.shishiTec.HiMaster.UI.Tantan;
...
/*        Object remove = mDatas.remove(viewHolder.getLayoutPosition());
       ...
       if (isLeftSwipe){
            ...
            //SwipeBean swipeBean = (SwipeBean) mDatas.get(viewHolder.getLayoutPosition());
            EventBus.getDefault().post(new TestEvent(ApiContact.KEY_ZUOHUADELTER,""));
        }else {
            EventBus.getDefault().post(new TestEvent(ApiContact.KEY_YOUHUAZAI,""));
            //Toast.makeText(mRv.getContext(), "右滑赞", Toast.LENGTH_SHORT).show();
        }
