package com.shishiTec.HiMaster.UI.Activity.mall.CalendarUtil;
...
public class CalendarCard extends View {
    ...
    public CalendarCard(Context context, OnCellClickListener listener, List<CalendarBean> calendarBeanList,CustomDate d) {
        super(context);
        this.context = context;
        ...
        for (int j = 0; j < TOTAL_ROW; j++) {
            ...
            for (int i = 0; i < TOTAL_COL; i++) {
                ...
                if (position >= firstDayWeek
                    ...
                    day++;
                    rows[j].cells[i] = new Cell(CustomDate.modifiDayForObject(mShowDate, day), State.NO_BUYDAY, i, j);
                    for (int w = 0; w < calendarBeanList.size(); w++) {
                        String s = calendarBeanList.get(w).getStart_date();
                        String s1 = DateUtils.getTime(mShowDate.getYear() + "-" + mShowDate.getMonth() + "-" + day);
                        if (s.equals(s1)) {
                            if(date!=null){
                                if(mShowDate.getYear() == date.getYear()){
                                    if(mShowDate.getMonth() == date.getMonth()){
                                        if(day == date.getDay()){
                                            rows[j].cells[i] = new Cell(CustomDate.modifiDayForObject(mShowDate, day), State.TODAY, i, j);
                                            break;
                                        }
                                    }
                                }
                            }
                            rows[j].cells[i] = new Cell(CustomDate.modifiDayForObject(mShowDate, day), State.BUYDAY, i, j);
                            break;
                        }
                    }
                    // 今天
...
//                }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mViewWidth = width;
        mViewHeight = R.dimen.gap_600px;//h
...
//            update();
    ...
    /**
     * @author wuwenjie 单元格的状态 当前月日期，过去的月的日期，下个月的日期,能买的日期，不能买的日期
     */
    enum State {
        TODAY, CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, BUYDAY, NO_BUYDAY, UNREACH_DAY;
    }
