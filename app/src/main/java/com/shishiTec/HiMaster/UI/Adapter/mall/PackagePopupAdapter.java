 ...
 */
public class PackagePopupAdapter extends RecyclerView.Adapter<PackagePopupAdapter.ItemViewHolder> {
    private Context context;
    private List<CourseDetailBean.CourseCfgBean> infoConfBeen;
    private OnItemClickListener listener;

    public PackagePopupAdapter(Context context, List<CourseDetailBean.CourseCfgBean> infoConfBeen) {
        super();
    ...
    public void onBindViewHolder(ItemViewHolder holder, final int position) {
        holder.tv_store_name.setText(infoConfBeen.get(position).getCustom_spec_name());
        holder.tv_address.setTextColor(Color.RED);
        holder.tv_address.setText("￥"+infoConfBeen.get(position).getPrice());
        holder.tv_address1.setVisibility(View.VISIBLE);
        holder.tv_address2.setVisibility(View.GONE);
        holder.tv_address1.setText("￥"+infoConfBeen.get(position).getShow_market_price());
        holder.tv_address1.getPaint().setFlags(Paint. STRIKE_THRU_TEXT_FLAG);
        /**
         * 调用接口回调
         */
